...
/**
 * Servlet implementation class postMyListController
 */
@WebServlet("/postMyList.po")
public class postMyListController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		ArrayList<Image2> list1 = new PostService().selectImage2Arraylist(pi);
		
		request.setAttribute("pi", pi);
		request.setAttribute("list", list);
		request.setAttribute("list1", list1);

		RequestDispatcher rd = request.getRequestDispatcher("views/post/postMyPost2.jsp");
